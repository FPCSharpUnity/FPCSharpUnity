...
namespace FPCSharpUnity.unity.Components.ui {
  ...
  public class CustomImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter {
    ...
    [SerializeField, ShowIf(nameof(showPreserveAspectEnvelope))] bool preserveAspectEnvelope;
    bool showPreserveAspectEnvelope => m_Type == Type.Simple && m_PreserveAspect;
    ...
    private Vector4 GetDrawingDimensions(bool shouldPreserveAspect, bool preserveAspectEnvelope = false) {
      ...
      if (shouldPreserveAspect && size.sqrMagnitude > 0.0f) {
        ...
        if ((spriteRatio > rectRatio) ^ preserveAspectEnvelope) {
    ...
    protected override void OnPopulateMesh(VertexHelper toFill) {
      ...
      switch (type) {
        case Type.Simple:
          GenerateSimpleSprite(toFill, m_PreserveAspect, preserveAspectEnvelope);
          break;
    ...
    /// </summary>
    void GenerateSimpleSprite(VertexHelper vh, bool lPreserveAspect, bool preserveAspectEnvelope = false) {
      Vector4 v = GetDrawingDimensions(lPreserveAspect, preserveAspectEnvelope);
